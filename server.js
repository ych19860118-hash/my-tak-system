...
app.post('/api/login', async (req, res) => {
    ...
    try {
        ...
        for (const s of sockets) {
            if (s.myName === uName) { isNameTaken = true; break; }
        }
    } catch (e) {}

    if (isNameTaken) {
        return res.json({ success: false, message: `登入失敗：暱稱「${uName}」目前正被房內其他在線人員使用中！` });
    }
